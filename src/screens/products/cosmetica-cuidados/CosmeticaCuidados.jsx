  ...
  import './CosmeticaCuidados.css';
  import { db } from '../../mainPage/firebaseConfig';
  import { collection, getDocs, query, where } from "firebase/firestore";
  import { useNavigate } from 'react-router-dom';

  function CosmeticaCuidado() {
    const [products, setProducts] = useState([]);
    const navigate = useNavigate();

    useEffect(() => {
      const fetchProducts = async () => {
        const q = query(collection(db, 'Productos'), where('categoria', '==', 'Cosmetica y cuidado'));
    ...
    return (
      <main className="main-container">
        <section className="featured-products">
          <h2>Cosmética y Cuidado Personal</h2>
  ...
  export default CosmeticaCuidado;
